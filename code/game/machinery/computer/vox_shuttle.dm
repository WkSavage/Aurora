...
/obj/machinery/computer/vox_stealth/attack_hand(mob/user as mob)
	...
	if(announce_vox_departure)
		user << "\red Shuttle stealth systems have been activated. The Hollowstone will not be warned of our arrival."
		announce_vox_departure = 0
	else
		user << "\red Shuttle stealth systems have been deactivated. The Hollowstone will be warned of our arrival."
...
/obj/machinery/computer/vox_station/proc/vox_move_to(area/destination as area)
	...
	if(announce_vox_departure)
		if(curr_location == locate(/area/shuttle/vox/station))
			command_alert("Attention, NTC Hollowstone, we just tracked a small target bypassing our defensive perimeter. Can't fire on it without hitting the station - you've got incoming visitors, like it or not.", "NSV Icarus")
		else if(dest_location == locate(/area/shuttle/vox/station))
			command_alert("Your guests are pulling away, NTC Hollowstone - moving too fast for us to draw a bead on them. Looks like they're heading out of Tau Ceti at a rapid clip.", "NSV Icarus")
