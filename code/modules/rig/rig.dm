...
/obj/item/rig
	name = "Basic Hardsuit Control Module"
	icon = 'icons/obj/rig_modules.dmi'
	desc = "A back-mounted hardsuit deployment and control mechanism. This is the bare bones model."
	...
	var/chest_type = /obj/item/rig/chest
	...
	var/obj/item/rig/foot = null            // Deployable boots, if any.
	var/obj/item/rig/chest                  // Deployable chestpiece, if any.
...
/obj/item/rig/examine()
	...
	usr << "[src.desc]"
	if(wearer)
		for(var/obj/item/piece in list(head,hand,chest,foot))
...
/obj/item/rig/New()
	...
	if(glove_type)
		hand = new glove_type(src)
		verbs |= /obj/item/rig/proc/toggle_gauntlets
	if(helm_type)
		head = new helm_type(src)
		verbs |= /obj/item/rig/head/proc/toggle_helmet
	if(boot_type)
		foot = new boot_type(src)
		verbs |= /obj/item/rig/foot/proc/toggle_boots
	if(chest_type)
		chest = new chest_type(src)
		if(allowed)
			chest.allowed = allowed
		chest.slowdown = offline_slowdown
		verbs |= /obj/item/rig/chest/proc/toggle_chest

	for(var/obj/item/piece in list(hand,head,foot,chest))
		if(!istype(piece))
			continue
		piece.canremove = 0
		...
		if(islist(armor)) piece.armor = islist(armor)

	update_icon(1)

/obj/item/rig/Destroy()
	for(var/obj/item/piece in list(hand,foot,head,chest))
...
/obj/item/rig/proc/suit_is_deployed()
	if(!istype(wearer) || src.loc != wearer || wearer.back != src)
		return 0
	if(helm_type && !(head && wearer.head == head))
		return 0
	if(glove_type && !(hand && wearer.gloves == hand))
		return 0
	if(boot_type && !(foot && wearer.shoes == foot))
		return 0
...
/obj/item/rig/proc/reset()
	offline = 2
	canremove = 1
	for(var/obj/item/piece in list(head,foot,hand,chest))
		if(!piece) continue
		piece.icon_state = "[initial(icon_state)]"
		if(airtight)
...
/obj/item/rig/proc/toggle_seals(var/mob/living/carbon/human/M,var/instant)
	...
	if(!failed_to_seal)
		...
		if(!M)
			...
			for(var/list/piece_data in list(list(M.shoes,foot,"boots",boot_type),list(M.gloves,hand,"gloves",glove_type),list(M.head,head,"helmet",helm_type),list(M.wear_suit,chest,"chest",chest_type)))
...
/obj/item/rig/process(var/obj/item/rig/R, var/charge = 10000)
	...
	if(!istype(wearer) || loc != wearer || wearer.back != src || canremove || !cell || R.cell.charge <= 0)
		if(!cell || R.cell.charge <= 0)
			if(electrified > 0)
				electrified = 0
			if(!offline)
				if(istype(wearer))
	...
	if(cell && R.cell.charge > 0 && electrified > 0)
		electrified--
...
/obj/item/rig/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1, var/nano_state = inventory_state, var/obj/item/rig/R)
	...
	data["head"] =    (head ? "[head.name]" : "None.")
	data["hand"] = (hand ? "[hand.name]" : "None.")
	data["foot"] =     (foot ?  "[foot.name]" :  "None.")
	data["chest"] =     (chest ?  "[chest.name]" :  "None.")

	data["charge"] =       cell ? round(R.cell.charge,1) : 0
	data["maxcharge"] =    cell ? R.cell.maxcharge : 0
	data["chargestatus"] = cell ? Floor((R.cell.charge/R.cell.maxcharge)*50) : 0
...
/obj/item/rig/rig/dropped(var/mob/user)
	..()
	for(var/piece in list("helmet","gauntlets","chest","boots"))
...
/obj/item/rig/emp_act(severity_class, var/obj/item/rig/R)
	//set malfunctioning
	...
	//drain some charge
	if(cell) R.cell.emp_act(severity_class + 15)
...
/*/obj/item/rig/rig/proc/forced_move(dir)
	if(locked_down)
		return 0
